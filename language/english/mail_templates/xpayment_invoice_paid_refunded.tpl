<p>Hello, {DRAWTO}</p>

<p>This is a notice that a paid invoice on {SITENAME} which was previously marked as "<span style='color: #b20000;'>refunded</span>"
    has now been marked as "<span style='color: #b20000;'>refunded</span>".
</p>

<p>Invoice #{INVOICENUMBER} <br><br>

    Amount: {AMOUNT} {CURRENCY}

</p>
<p>
    If this was a mistake please contact us immediately!
</p>
<p>You can view the invoice at <br><br> {INVURL}</p>
<p>You can view a printable version of your invoice at <br><br> {PDFURL}</p>
<p>
    ------------------------------------------------------
</p>
<p>Thank you for using {SITENAME}'s services</p>
<p>Many Thanks</p>
<p>{SITENAME}</p>
<p>{SITEURL}</p>
