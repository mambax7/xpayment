 ...
 * @param string $sort
 * @param string $fct
 * @return string|XoopsFormElementTray|XoopsFormSelectInvoiceMode|XoopsFormSelectInvoiceRemittion
 */
...
function xpayment_getFilterElement($filter, $field, $sort = 'created', $fct = 'invoice')
{
    ...
    require_once __DIR__ . '/xpayment.objects.php';
    $ele = '';
    switch ($field) {
        case 'mode':
...
function xpayment_install_gateway($class)
{
    ...
    if (0 == $gatewaysHandler->getCount(new Criteria('class', $class))) {
        ...
        if (!empty($gateway)) {
            $gateways = $gatewaysHandler->create();
            ...
            if ($gid = $gatewaysHandler->insert($gateways)) {
...
function xpayment_update_gateway($class)
{
    ...
    if (1 == $gatewaysHandler->getCount(new Criteria('class', $class))) {
        ...
        if (!empty($gateway)) {
            $gatewaysObjs = $gatewaysHandler->getObjects(new Criteria('class', $class));
            ...
            if ($gid = $gatewaysHandler->insert($gateways)) {
                foreach ($gateway['options'] as $refereer => $data) {
                    ...
                    if (0 == $gateways_optionsHandler->getCount($criteria)) {
                        ...
                        $optionObjs = $gateways_optionsHandler->getObjects($criteria);
                        $option     = $optionObjs[0];
                        $option->setVar('name', $data['name']);
                        $gateways_optionsHandler->insert($option);
                    }
