...
<div style='width:100%; clear:both;'>
    ...
    <div>
        <div style='width:190px;float:left;'><{$smarty.const._XPY_MF_TOTALINTEREST}> :</div>
    ...
    <{if $invoice.did != 0}>
        <div>
            <div style='width:190px;float:left;'><{$smarty.const._XPY_MF_DISCOUNTPERCENTILE}> :</div>
            <div><strong><{$invoice.discount}>%</strong></div>
        </div>
        <div>
            <div style='width:190px;float:left;'><{$smarty.const._XPY_MF_DISCOUNTAMOUNT}> :</div>
            <div><strong><{$invoice.discount_amount}> <{$invoice.currency}></strong></div>
        </div>
    <{/if}>
</div>
...
<table>
    ...
    <{if $invoice.did != 0}>
        <tr class="foot">
            <td colspan="4">&nbsp;</td>
            <td><{$smarty.const._XPY_MF_DISCOUNT_TD}></td>
...
<{if $payment_markup}>
<h2><{$smarty.const._XPY_MF_APPLY_DISCOUNT_H2}></h2>
<p><{$smarty.const._XPY_MF_APPLY_DISCOUNT_P}></p>
<{$discount_form}>
<h2><{$smarty.const._XPY_MF_MAKEPAYMENT_H2}></h2>
<p><{$smarty.const._XPY_MF_MAKEPAYMENT_P}></p>
<h3><{$smarty.const._XPY_MF_MAKEPAYMENT_MANUAL}></h2>
    <p>The following details will allow you to make a manual payment with the bank. Send the total amount of
        <strong><{$invoice.grand}> <{$invoice.currency}></strong> to:<b><{$xoConfig.manual}><b><b>Quote
                    Reference Number: <strong><{$xoConfig.manualcode}><{$invoice.iid}></strong></p>
    <h3><{$smarty.const._XPY_MF_MAKEPAYMENT_ONLINE}></h2>
        <div><{$smarty.const._XPY_MF_MAKEPAYMENT_SELECTGATEWAY}>: <{$payment_markup}></div>
        <div id='gateway_html'></div>
        <script type="text/javascript">ChangeGateway('gid');</script>
        <{/if}>
