        ...
        */
        ...
        public function _destroy($destroyall=false, $preserve_objcopy=false)
        {
            ...
            foreach (array_keys(get_object_vars($this)) as $val) {
                if ($destroyall or (
                    ('internal_encoding' != $val)
                    and ('state' != $val)
                    and ('bufferlen' != $val)
                    and ('buffer' != $val)
                    and ('diskcache' != $val)
                    and ('sign' != $val)
                    and ('signature_data' != $val)
                    and ('signature_max_length' != $val)
                    and ('byterange_string' != $val)
                    )) {
            ...
            for ($i=0; $i < $numchars; ++$i) {
                if (K_RLE == $ta[$i]) {
                    ...
                    if (count($remember)) {
                        ...
                        if ((K_RLE == $remember[$last]['num']) or
                            (K_LRE == $remember[$last]['num']) or
                            (K_RLO == $remember[$last]['num']) or
                            (K_LRO == $remember[$last]['num'])) {
                ...
                for ($i=0; $i < $numchars; ++$i) {
                    ...
                    if ('AL' == $unicode[$thischar['char']]) {
                        ...
                        if ((false !== $prevchar) and (false !== $nextchar) and
                                                      (('AL' == $unicode[$prevchar['char']]) or ('NSM' == $unicode[$prevchar['char']])) and
                                                      (('AL' == $unicode[$nextchar['char']]) or ('NSM' == $unicode[$nextchar['char']])) and
    ...
    Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
            ...
            while ($elkey < $maxel) {
                ...
                if (preg_match($tagpattern, $element)) {
                    ...
                    if ('/' == $element{0}) {
                        ...
                        if ((('ul' == $dom[$key]['value']) or ('ol' == $dom[$key]['value']) or ('dl' == $dom[$key]['value']))
                            and (!isset($dom[$key]['align']) or $this->empty_string($dom[$key]['align']) or ('J' != $dom[$key]['align']))) {
                            if ($this->rtl) {
                                $dom[$key]['align'] = 'R';
                            } else {
                                $dom[$key]['align'] = 'L';
                        ...
                        if (('h' == $dom[$key]['value']{0}) and (intval($dom[$key]['value']{1}) > 0) and (intval($dom[$key]['value']{1}) < 7)) {
        ...
        public function writeHTML($html, $ln=true, $fill=false, $reseth=false, $cell=false, $align='')
        {
            ...
            while ($key < $maxel) {
                if ($dom[$key]['tag'] and isset($dom[$key]['attribute']['pagebreak'])) {
                    ...
                    if ((('left' == $dom[$key]['attribute']['pagebreak']) and (((!$this->rtl) and (0 == ($this->page % 2))) or (($this->rtl) and (0 != ($this->page % 2)))))
                        or (('right' == $dom[$key]['attribute']['pagebreak']) and (((!$this->rtl) and (0 != ($this->page % 2))) or (($this->rtl) and (0 == ($this->page % 2)))))) {
                        $this->AddPage();
                    }
                }
                                ...
                                if (preg_match_all('/\[\(([^\)]*)\)\]/x', $pmidtemp, $lnstring, PREG_PATTERN_ORDER)) {
                                    ...
                                    while (1 == preg_match('/([0-9\.\+\-]*)[\s](Td|cm|m|l|c|re)[\s]/x', $pmid, $strpiece, PREG_OFFSET_CAPTURE, $offset)) {
                                        ...
                                        if ((($epsposbeg > 0) and ($epsposend > 0) and ($offset > $epsposbeg) and ($offset < $epsposend))
                                            or ((false === $epsposbeg) and ($epsposend > 0) and ($offset < $epsposend))) {
                                            // shift EPS images
        ...
        protected function openHTMLTagHandler(&$dom, $key, $cell=false)
        {
            ...
            if ($dom[$key]['self'] and isset($dom[$key]['attribute']['pagebreakafter'])) {
                ...
                if ((('left' == $pba) and (((!$this->rtl) and (0 == ($this->page % 2))) or (($this->rtl) and (0 != ($this->page % 2)))))
                    or (('right' == $pba) and (((!$this->rtl) and (0 != ($this->page % 2))) or (($this->rtl) and (0 == ($this->page % 2)))))) {
                    $this->AddPage();
                }
            }
        }
