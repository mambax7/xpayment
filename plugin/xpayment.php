...
function PaidXpaymentHook($invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_PAID, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function UnpaidXpaymentHook($invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_UNPAID, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function CancelXpaymentHook($invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_CANCEL, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function DiscountedPaidXpaymentHook($invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_PAID + _XPY_ENUM_REMITTION_DISCOUNTED, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function DiscountedUnpaidXpaymentHook($invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_UNPAID + _XPY_ENUM_REMITTION_DISCOUNTED, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function DiscountedCancelXpaymentHook($invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_CANCEL + _XPY_ENUM_REMITTION_DISCOUNTED, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function NonePaidXpaymentHook($invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_PAID + _XPY_ENUM_REMITTION_NONE, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function NoneUnpaidXpaymentHook($invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_UNPAID + _XPY_ENUM_REMITTION_NONE, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function NoneCancelXpaymentHook($invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_CANCEL + _XPY_ENUM_REMITTION_NONE, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function PendingPaidXpaymentHook($invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_PAID + _XPY_ENUM_REMITTION_PENDING, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function PendingUnpaidXpaymentHook($invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_UNPAID + _XPY_ENUM_REMITTION_PENDING, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function PendingCancelXpaymentHook($invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_CANCEL + _XPY_ENUM_REMITTION_PENDING, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function NoticePaidXpaymentHook($invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_PAID + _XPY_ENUM_REMITTION_NOTICE, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function NoticeUnpaidXpaymentHook($invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_UNPAID + _XPY_ENUM_REMITTION_NOTICE, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function NoticeCancelXpaymentHook($invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_CANCEL + _XPY_ENUM_REMITTION_NOTICE, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function CollectPaidXpaymentHook($invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_PAID + _XPY_ENUM_REMITTION_COLLECT, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function CollectUnpaidXpaymentHook($invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_UNPAID + _XPY_ENUM_REMITTION_COLLECT, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function CollectCancelXpaymentHook($invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_CANCEL + _XPY_ENUM_REMITTION_COLLECT, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function FraudPaidXpaymentHook($invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_PAID + _XPY_ENUM_REMITTION_FRAUD, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function FraudUnpaidXpaymentHook($invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_UNPAID + _XPY_ENUM_REMITTION_FRAUD, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function FraudCancelXpaymentHook($invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_CANCEL + _XPY_ENUM_REMITTION_FRAUD, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function SettledPaidXpaymentHook($invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_PAID + _XPY_ENUM_REMITTION_SETTLED, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function SettledUnpaidXpaymentHook($invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_UNPAID + _XPY_ENUM_REMITTION_SETTLED, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function SettledCancelXpaymentHook($invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_CANCEL + _XPY_ENUM_REMITTION_SETTLED, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function PurchasedPaidXpaymentItemHook($item, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_PAID + _XPY_ENUM_ITEMMODE_PURCHASED, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function PurchasedUnpaidXpaymentItemHook($item, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_UNPAID + _XPY_ENUM_ITEMMODE_PURCHASED, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function PurchasedCancelXpaymentItemHook($item, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_CANCEL + _XPY_ENUM_ITEMMODE_PURCHASED, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function RefundedPaidXpaymentItemHook($item, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_PAID + _XPY_ENUM_ITEMMODE_REFUNDED, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function RefundedUnpaidXpaymentItemHook($item, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_UNPAID + _XPY_ENUM_ITEMMODE_REFUNDED, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function RefundedCancelXpaymentItemHook($item, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_CANCEL + _XPY_ENUM_ITEMMODE_REFUNDED, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function UndeliveredPaidXpaymentItemHook($items, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_PAID + _XPY_ENUM_ITEMMODE_UNDELIVERED, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function UndeliveredUnpaidXpaymentItemHook($item, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_UNPAID + _XPY_ENUM_ITEMMODE_UNDELIVERED, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function UndeliveredCancelXpaymentItemHook($item, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_CANCEL + _XPY_ENUM_ITEMMODE_UNDELIVERED, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function DamagedPaidXpaymentItemHook($item, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_PAID + _XPY_ENUM_ITEMMODE_DAMAGED, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function DamagedUnpaidXpaymentItemHook($item, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_UNPAID + _XPY_ENUM_ITEMMODE_DAMAGED, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function DamagedCancelXpaymentItemHook($item, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_CANCEL + _XPY_ENUM_ITEMMODE_DAMAGED, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function ExpressPaidXpaymentItemHook($item, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_PAID + _XPY_ENUM_ITEMMODE_EXPRESS, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function ExpressUnpaidXpaymentItemHook($item, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_UNPAID + _XPY_ENUM_ITEMMODE_EXPRESS, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function ExpressCancelXpaymentItemHook($item, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_CANCEL + _XPY_ENUM_ITEMMODE_EXPRESS, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function PaymentPaidXpaymentTransactionHook($transaction, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_PAID + _XPY_ENUM_TRANSACTION_PAYMENT, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function PaymentUnpaidXpaymentTransactionHook($transaction, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_UNPAID + _XPY_ENUM_TRANSACTION_PAYMENT, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function PaymentCancelXpaymentTransactionHook($transaction, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_CANCEL + _XPY_ENUM_TRANSACTION_PAYMENT, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function RefundPaidXpaymentTransactionHook($transaction, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_PAID + _XPY_ENUM_TRANSACTION_REFUND, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function RefundUnpaidXpaymentTransactionHook($transaction, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_UNPAID + _XPY_ENUM_TRANSACTION_REFUND, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function RefundCancelXpaymentTransactionHook($transaction, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_CANCEL + _XPY_ENUM_TRANSACTION_REFUND, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function PendingPaidXpaymentTransactionHook($transaction, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_PAID + _XPY_ENUM_TRANSACTION_PENDING, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function PendingUnpaidXpaymentTransactionHook($transaction, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_UNPAID + _XPY_ENUM_TRANSACTION_PENDING, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function PendingCancelXpaymentTransactionHook($transaction, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_CANCEL + _XPY_ENUM_TRANSACTION_PENDING, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function NoticePaidXpaymentTransactionHook($transaction, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_PAID + _XPY_ENUM_TRANSACTION_NOTICE, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function NoticeUnpaidXpaymentTransactionHook($transaction, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_UNPAID + _XPY_ENUM_TRANSACTION_NOTICE, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function NoticeCancelXpaymentTransactionHook($transaction, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_CANCEL + _XPY_ENUM_TRANSACTION_NOTICE, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function OtherPaidXpaymentTransactionHook($transaction, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_PAID + _XPY_ENUM_TRANSACTION_OTHER, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function OtherUnpaidXpaymentTransactionHook($transaction, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_UNPAID + _XPY_ENUM_TRANSACTION_OTHER, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function OtherCancelXpaymentTransactionHook($transaction, $invoice)
{
    if ($invoice->getVar('iid') > 0) {
        ...
        $grouppermHandler = xoops_getHandler('groupperm');
        $xpMod            = $moduleHandler->getByDirname('xpayment');
        if ($grouppermHandler->checkRight('email', _XPY_ENUM_MODE_CANCEL + _XPY_ENUM_TRANSACTION_OTHER, $ttlgroups, $xpMod->getVar('mid'))) {
            if (!$xoopsMailer->send()) {
                return false;
            } else {
                return true;
...
function setEmailXpaymentReceiever($xoopsMailer, $invoice, $itemid)
{
    ...
    foreach ($invoice->getVar('broker_uids') as $id => $uid) {
        ...
        if ($grouppermHandler->checkRight('email', $itemid, $groups, $xpMod->getVar('mid'))) {
    ...
    $user = $userHandler->get($invoice->getVar('user_uid'));
...
function setEmailXpaymentTags($xoopsMailer, $invoice, $item, $transaction)
{
    ...
    $xoopsMailer->assign('PAID', $invoice_transactionsHandler->sumOfGross($invoice->getVar('iid'))) + $gross;
    $xoopsMailer->assign('LEFT', $invoice->getVar('grand') - ($invoice_transactionsHandler->sumOfGross($invoice->getVar('iid')) + $gross));
    ...
    $items                = $invoice_itemsHandler->getObjects(new Criteria('iid', $invoice->getVar('iid')), false, true);
    $i                    = 0;
    ...
    $profile = $moduleHandler->getByDirname('profile');
    ...
    foreach ($invoice->getVar('broker_uids') as $id => $uid) {
        ...
        if (is_object($user)) {
            ...
            if (is_object($profile)) {
                $userprofile = $profileHandler->get($uid);
