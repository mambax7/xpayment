...
class GoogleCart
{
     ...
     *                            defaults to 'USD'
     */
    public function __construct($id, $key, $server_type = 'sandbox', $currency = 'USD')
    {
        $this->merchant_id  = $id;
     ...
     * @param bool|string $tax_option true if the merchant has to do tax calculations.
     *                                defaults to false.
     * @param bool|string $coupons    true if the merchant accepts discount coupons.
     *                                defaults to false.
     * @param bool|string $gift_cert  true if the merchant accepts gift certificates.
     *                                defaults to false.
     * @return void
     */
     ...
     * @internal param attributes $tracking_attr_types to be tracked, one of
     *                             ('buyer-id',
     *                             'order-id',
     *                             'order-subtotal',
    ...
    public function getXML()
    {
        ...
        foreach ($this->item_arr as $item) {
            ...
            if ($item->digital_content) {
                ...
                } elseif (!empty($item->digital_description)) {
                    $xml_data->Element('description', substr($item->digital_description, 0, MAX_DIGITAL_DESC));
                } else {
                    ...
                    if ('google' === $sub->subscription_type) {
                        ...
                        if ($recurrent_item->digital_content) {
                            ...
                            if (!empty($recurrent_item->digital_url)) {
                                $xml_data->Element('description', substr($recurrent_item->digital_description, 0, MAX_DIGITAL_DESC));
                                ...
                                $xml_data->Element('email-delivery', $this->_GetBooleanValue($recurrent_item->email_delivery, 'true'));
                            }
                            $xml_data->Pop('digital-content');
                        }
    ...
    public function CheckoutServer2ServerButton(
        ...
        if ('text' === $this->variant) {
            ...
            $data .= "<div><img alt=\"Checkout\" src=\""
                     . ''
                     . $this->server_url
                     . 'buttons/checkout.gif?merchant_id='
                     . ''
                     . $this->merchant_id
                     . '&amp;w='
                     . $width
    ...
    public function CheckoutButtonNowCode(
        ...
        if ('text' === $this->variant) {
            $data .= "<div align=\"center\"><form method=\"post\" action=\"" . $this->checkout_url . "\"" . ($this->googleAnalytics_id ? " onsubmit=\"setUrchinInputCode();\"" : '') . ">
                ...
                src=\"" . $this->server_url . 'buttons/buy.gif?merchant_id=' . $this->merchant_id . '&amp;w=' . $width . '&amp;h=' . $height . '&amp;style=' . $style . '&amp;variant=' . $this->variant . '&amp;loc=' . $loc . "\"
                style=\"height:" . $height . 'px;width:' . $width . "px\">";
        ...
        foreach ($data as $tag_name => $tag) {
            ...
            if (is_array($tag)) {
                ...
                if (!$this->is_associative_array($data)) {
                    ...
                    if (isset($this->multiple_tags[$tag_name]) && $this->is_associative_array($tag)
