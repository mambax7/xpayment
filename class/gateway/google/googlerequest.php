...
class GoogleRequest
{
     ...
     * @param float|string $amount       the amount to be charged, if empty the whole
     ...
     * @return array  the status code and body of the response
     */
    public function SendChargeAndShipOrder($google_order, $tracking_data = null, $amount = null)
    {
        $postargs = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>
                  <charge-and-ship-order xmlns=\"" . $this->schema_url . "\" google-order-number=\"" . $google_order . "\">";
        if (isset($amount)) {
            $postargs .= "<amount currency=\"" . $this->currency . "\">" . $amount . '</amount>';
        }
        if (isset($tracking_data)) {
            ...
            foreach ($tracking_data as $tracking_code => $carrier) {
                $postargs .= '<tracking-data>' . '<carrier>' . $carrier . '</carrier>' . '<tracking-number>' . $tracking_code . '</tracking-number>' . '</tracking-data>';
            }
            $postargs .= '</tracking-data-list>';
        }
     ...
     * @return array  the status code and body of the response
     */
    public function SendRecurrenceRequest($google_order, $item_arr)
    {
        $postargs = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>
      <create-order-recurrence-request xmlns=\"" . $this->schema_url . "\" google-order-number=\"" . $google_order . "\">
      <shopping-cart>
        <items>";
     ...
     * @param bool   $timeout
     * @return array the status code and body of the response
     * @internal param string $send_email whether Google should send an email to
