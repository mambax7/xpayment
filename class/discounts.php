...
class XpaymentDiscounts extends XoopsObject
{
    ...
    public function toArray()
    {
        ...
        if ($this->getVar('uid') > 0) {
            ...
            $user        = $userHandler->get($this->getVar('uid'));
...
class XpaymentDiscountsHandler extends XoopsPersistableObjectHandler
{
    ...
    public function __construct(XoopsDatabase $db)
    {
        ...
        $this->_mod       = $moduleHandler->getByDirname('xpayment');
        $this->_modConfig = $configHandler->getConfigList($this->_mod->getVar('mid'));
    }
    ...
    private function getCode($prefix, $length = 42, $num = 6)
    {
        ...
        while (strlen($code) < 48 && count($code) > 0) {
            $code .= $base[mt_rand(0, count($base) - 1)];
        }
    ...
    public function getUsersByGroup($group_id, $logged_in_since = 0, $registered_since = 0, $asobject = true)
    {
        ...
        if (!$asobject) {
            ...
            foreach ($user_ids as $u_id) {
                ...
                if (is_object($user)) {
                    if (($logged_in_since > 0 && $user->getVar('last_login') > $logged_in_since)
                        && ($registered_since > 0
    ...
    public function sendDiscountCode($email, $validtill, $redeems, $discount, $prefix, $uid = 0, $sendemail = true)
    {
    ...
    public function insert(XoopsObject $object, $force = true, $sendemail = true)
    {
        // xoops_load('xoopsmailer');
        xoops_loadLanguage('admin', 'xpayment');
